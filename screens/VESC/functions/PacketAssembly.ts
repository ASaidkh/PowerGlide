...
export class PacketAssembly {
  ...
  static createPacket(commandId: number, data: number[] = []): Buffer {
    const payload = [commandId, ...data];
    const payloadBuffer = Buffer.from(payload);
    const crc = this.crc16(payloadBuffer);
    
      // Create the full packet
      const packet = Buffer.concat([
        Buffer.from([2, payload.length]),  // Start byte and length byte
        payloadBuffer,                     // Payload bytes
        Buffer.from([crc >> 8, crc & 0xFF, 3])  // CRC bytes and end byte
      ]);
    
      // Log the packet in the requested format
      const commandIdByte = `[${packet[0]}]`; // Command ID byte
      const dataBytes = `[${payload.slice(1).join(', ')}]`; // Data bytes
  ...
  static assemblePacket(chunks: any[]): Buffer {
